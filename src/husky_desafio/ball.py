...
class Camera:
  ...
  def __init__(self):
    ...
    self.pub_cancel_move = rospy.Publisher("/move_base/cancel", GoalID, queue_size=1)
    ...
    for index, obj_cnt in enumerate(cnt_yellow):
      ...
      if(len(contours_poly[index]) > 10):
        ...
        if self.flag_finished:
          cv2.circle(cv2_frame, (int(centers[index][0]), int(centers[index][1])), int(radius[index]), (0, 0, 255), 5) 
          cv2.putText(cv2_frame, 'HUSKY REACHED ITS GOAL!', (20, 130), font, 2, (0, 0, 255), 5)
          print('HUSKY REACHED ITS GOAL!')
